...
const renderHtmlTestDetail = (data, subjectId) => {
  console.log('renderHtmlTestDetail -> data', data);
  let padding = '14px';
  ...
  for (let i = 0; i < totalAll; i++) {
    ...
    if (typeData == 1) {
      ...
      html += `<div style="overflow:auto;padding: 10px 15px;border-bottom: 1px solid #eaeaea" class="q_material">
              ${data[i].dataMaterial.contentHtml}
             </div>`;
      let totalMaterial = Object.keys(
        !!data[i].dataMaterial.data && data[i].dataMaterial.data,
      ).length;
      for (let k = 0; k < totalMaterial; k++) {
        let dataMaterial = data[i].dataMaterial.data[k];
        let displayFlash = 'display: inline-block';
        if (
          dataMaterial.flashcardId == null ||
          dataMaterial.flashcardId == 'null' ||
          dataMaterial.flashcardId == ''
        ) {
        ...
        if (dataMaterial.rightAnswer) {
          ...
          Object.keys(!!dataMaterial.userOptionId && dataMaterial.userOptionId)
            .length > 0 ||
        ...
        if (typeAnswer > 2) {
          html += `<div style="margin-left:15px;margin-right:15px;">
            <textarea placeholder="Nhập câu trả lời" disabled type="text" name="in" style="width:96%;min-height:80px;max-height:140px;padding:5px;margin:15px 0" value="";> ${
              dataMaterial.userOptionText || ''
            }</textarea>
            </div>`;
        } else if (typeAnswer <= 1) {
          let countOption = Object.keys(
            !!dataMaterial.options && dataMaterial.options,
          ).length;
          for (j = 0; j < countOption; j++) {
            ...
            if (dataMaterial.userOptionId.indexOf(idOption) > -1) {
              bgAnswer = '#4CAF79';
              bgContent = '#eff9e8';
            }
            html += `<div style="padding:${padding} 0; min-height:40px; border-radius: 4px;border-top:1px solid ${borderContent};background:${bgContent}; overflow: hidden; display: flex; align-items: center;">
              ...
              <span  style="color: #fff;padding: 4px 8px;background: ${bgAnswer};border-radius: 100px;font-size:12px;">${getAnswerName(
              j,
            )}</span>
              </span>
              <span class="opcontent" style="width:86% !important;display:inline-block !important;float: left !important;overflow: auto;">${
                dataMaterial.options[j].content
              }</span>
              </div>
              `;
          }
        ...
        if (
          Object.keys(!!dataMaterial.userOptionId && dataMaterial.userOptionId)
            .length > 0 ||
          dataMaterial.userOptionText != null
        ) {
          html += `
      ...
      let displayFlash = 'display: inline-block';
      if (
        dataStandard.flashcardId == null ||
        dataStandard.flashcardId == 'null' ||
        dataStandard.flashcardId == ''
      ) {
        displayFlash = 'display: none';
      }
      ...
      if (dataStandard.rightAnswer) {
        ...
        Object.keys(!!dataStandard.userOptionId && dataStandard.userOptionId)
          .length > 0 ||
      ...
      html += `<div style="padding:10px 15px">
      <span style="font-weight: bold;font-size: 14px;color: #9B9B9B">Câu ${
        dataStandard.stepIndex + 1
      }</span>
      ${rightAnswer2}
      <span style="color:#9B9B9B;margin: 0 5px;font-size: 14px;">|</span>
      <span style="color:#9B9B9B;font-size: 14px">#${
        dataStandard.numberQuestion
      }</span>
      <span style="float: right" id="arrayWarn${
        dataStandard.stepIndex
      }" data-numberQuestion="${dataStandard.numberQuestion}">
          <span style="padding-right: 5px" onclick="myWarningFunction('${
            dataStandard.numberQuestion
          }')">
              <i class="fa fa-warning" style="color:#9B9B9B;font-size: 14px;"></i>
      ...
      if (dataStandard.typeAnswer <= 1) {
        let countOption = Object.keys(
          !!dataStandard.options && dataStandard.options,
        ).length;
        for (j = 0; j < countOption; j++) {
